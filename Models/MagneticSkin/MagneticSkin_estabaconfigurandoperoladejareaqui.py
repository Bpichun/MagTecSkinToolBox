...
class FitnessEvaluationController(BaseFitnessEvaluationController):   
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.current_stretch = self.stretch_levels[0]
    ...
    def onAnimateBeginEvent(self, event):
        ...
        if self.t == 0:
            ...
            value = list(grab.translation.value)
            value[0] = displacement

            grab.translation.value = value
            grab.reinit()

            print(f"[Stretch] {self.current_stretch*100:.0f}%")

            model_spring = self.ModelNode
            spring = model_spring.getObject('BaseFixSpring')
            spring.findData('stiffness').value = [0]
        ...
        if self.t > 30:
            ...
            if self.current_point >= len(self.trayectoria_5puntos):
                print(f"Trajectory ended for stretch {self.current_stretch*100:.0f}%")

                self.current_stretch_idx += 1

                if self.current_stretch_idx < len(self.stretch_levels):
                    # Pasar al siguiente stretch
                    ...
                    self.waiting = False
                    self.offset = None
                    # self.data_MagneticField.append([])  # nuevo bloque de datos
                    self.t = 0
        ...
        # print('MagnetPosition', MagnetPosition)
        ...
        if self.offset is None and self.t == 30:
            self.offset = GlobalMagneticField_index_real.copy()
        ...
        if self.waiting:
            ...
            if self.frames_counter >= self.frames_to_wait:  
                ...
                if self.current_stretch_idx == 0:
                    self.data_MagneticField_000pct.append(GlobalMagneticField_index_real.copy())
                    print(f"guardado 000pct:{ GlobalMagneticField_index_real.copy()}")
                elif self.current_stretch_idx == 1:
                    self.data_MagneticField_010pct.append(GlobalMagneticField_index_real.copy())
                    print(f"guardado 010pct:{ GlobalMagneticField_index_real.copy()}")
                elif self.current_stretch_idx == 2: 
                    self.data_MagneticField_020pct.append(GlobalMagneticField_index_real.copy())
                    print(f"guardado 020pct:{ GlobalMagneticField_index_real.copy()}")
        ...
        if self.current_iter == self.max_iter -1:
            ...
            for i in range(len(current_objectives_names)):
                ...
                # metrics_per_stretch = []
                # if "MagnetSensitivity_000pct" == current_objective_name:
                ...
                #     self.objectives.append(metric_000pct)

                # if "MagnetSensitivity_010pct" == current_objective_name:
                ...
                #     self.objectives.append(metric_010pct)

                # if "MagnetSensitivity_020pct" == current_objective_name:
                ...
                #     self.objectives.append(metric_020pct)
