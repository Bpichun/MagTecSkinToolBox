...
class Config(GmshDesignOptimization):
    ...
    def init_model_parameters(self):
        ...
        # --- Magnet density ---
        self.MagnetDensity_x = 0.007432475660847371
        self.MagnetDensity_y = 0.046742802646230194
        self.MagnetGridOrientation = 40.71916396935461  # degrees
        self.delta_x = -0.4651634081806989
        self.delta_y = 0.3208668812634635
        ...
        self.SensorDensity_x = 0.009
        self.SensorDensity_y = 0.03928 
        self.SensorGridOrientation = 45  # degrees
        self.delta_x_sensor = -0.5
        self.delta_y_sensor = -0.5

        self.spacing_x_sensor = np.sqrt(1.0 / self.SensorDensity_x)
        self.spacing_y_sensor = np.sqrt(1.0 / self.SensorDensity_y)
        self.grid_xy_sensor = generate_infinite_grid(self.spacing_x_sensor, self.spacing_y_sensor, n=30, angle_deg=self.SensorGridOrientation, origin=(self.delta_x_sensor, self.delta_y_sensor))
        ...
        self.SensorGridPoints = cut_grid(self.grid_xy_sensor, length=self.Length, width=self.Width, margin_x=5, margin_y=5)
        ...
        # self.max_dx = min(self.spacing_x, self.spacing_y) * 0.25
        self.max_dx = 0.5
    ...
    def get_design_variables(self):
        return {
            ...
            "delta_x": [self.delta_x, -0.0, 0.5],
            "delta_y": [self.delta_y, -0.0, 0.5],
            # "NumberSensors": [self.NumberSensors, 1, 15.0],

            # "SensorDensity_x": [self.SensorDensity_x, 0.009, 0.011], 
            # "SensorDensity_y": [self.SensorDensity_y, 0.034, 0.04], 
            # "SensorGridOrientation": [self.SensorGridOrientation, 0.0, 40],
            # "delta_x_sensor": [self.delta_x_sensor, -0.5, 0.5],
            # "delta_y_sensor": [self.delta_y_sensor, -0.5, 0.5],
        }
    

    def get_objective_data(self):
        return {
        "MagnetNumber": ["minimize", 130],
        # "SensorNumber": ["minimize", 130],
        # "MagneticSensitivity_000pct": ["maximize", 500],     
        # "MagneticSensitivity_010pct": ["maximize", 500],     
        # "MagneticSensitivity_020pct": ["maximize", 500],
    ...
    def get_assessed_together_objectives(self):
        ...
        return [[ "MagnetNumber", "MagneticSensitivity"]]                    
    ...
    def set_design_variables(self, new_values):
        ...
        self.spacing_x_sensor = np.sqrt(1.0 / self.SensorDensity_x)
        self.spacing_y_sensor = np.sqrt(1.0 / self.SensorDensity_y)
        self.grid_xy_sensor = generate_infinite_grid(self.spacing_x_sensor, self.spacing_y_sensor,
                                                     n=30, angle_deg=self.SensorGridOrientation, 
                                                     origin=(self.delta_x_sensor, self.delta_y_sensor))
